...
function sendEmails(emails) {
  return new Promise((resolve, reject) => {
    try {
      ...
      reject(Error(noTokenErrorMessage));
      return;
    }
...
${email.body.replace(/\n/g, '<br>')}`;
      ...
      .catch(() => reject(Error(gettingProfileErrorMessage)));
  })
    .catch((err) => {
      console.log(err);

      if (err.message === noTokenErrorMessage || err.message === gettingProfileErrorMessage) {
        ...
        throw Error(notAuthedErrorMessage);
      } else {
        throw err;
      }
    });
