...
async function getAppointments(staffId, startDateObj, endDateObj) {
  const sessionId = await getSession();
  const response = await rp({
    uri: `https://my.setmore.com/fetchAllAppoointmentsByDate/${staffId}/${startDateObj.format(config.setmoreRequestDateTimeFormat)}/${moment(endDateObj).add(1, 'days').format(config.setmoreRequestDateTimeFormat)}`,
    jar: getCookieJar(sessionId)
  });
