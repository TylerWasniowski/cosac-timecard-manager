...
function evaluateHoursWorked(
  ...
  const extendedSlotBlockers = slotBlockers
    .filter(slotBlocker => slotBlocker.Comments.match(setmoreExtendedHoursRegex));
  const adminSlotBlockers = slotBlockers
    .filter(slotBlocker => slotBlocker.Comments.match(setmoreAdminHoursRegex));
...
function findInsertIndex(intervals, timeObj) {
  const indexIntervalPair = Array.from(intervals.entries())
    .find(pair => timeObj.isAfter(pair[1].startObj));

  return indexIntervalPair ? indexIntervalPair[0] + 1 : 0;
}
