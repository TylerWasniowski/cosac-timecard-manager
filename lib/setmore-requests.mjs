...
async function getNotifications(startIndex) {
  return requestData(`https://my.setmore.com/setmoreNotification?staffKey=${getSetmoreId()}&startIndex=${startIndex}&accessType=Admin%20Access&isAdminLogin=true`);
}
...
function getSetmoreId() {
  return JSON.parse(process.env.tutors)
    .find(tutor => tutor.email.toLowerCase() === setmoreUsername.toLowerCase())
    .setmoreId;
}
